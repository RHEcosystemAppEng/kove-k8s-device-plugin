...
func NewGenericPlugin(ds *DeviceSpec, pluginDir string, logger log.Logger) Plugin {
	if logger == nil {
	...
	return NewPlugin(ds.Name, pluginDir, gp, logger)
}

// NewPlugin creates a new instance of a device plugin.
func NewPlugin(resource, pluginDir string, dps v1beta1.DevicePluginServer, logger log.Logger) Plugin {
	if logger == nil {
		logger = log.NewNopLogger()
	}
	p := &plugin{
