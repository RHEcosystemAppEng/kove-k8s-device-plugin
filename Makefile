...
BINS := $(addprefix bin/$(ARCH)/,kove-k8s-device-plugin)
PROJECT := kove-k8s-device-plugin
...
tmp/help.txt: bin/$(ARCH)/kove-k8s-device-plugin
	mkdir -p tmp
	bin/$(ARCH)/kove-k8s-device-plugin --help 2>&1 | head -n -1 > $@
